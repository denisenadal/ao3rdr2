...
const FicTable = ({ fics, updateSelectedFic, toggleModal, settings, readyState }: tableProps) => {
  ...
  function handleCellEdit(row: Fic, value: string, column: TableColumn<Fic>) {
    ...
    if (field !== "personal_tags") return;
    const newTags = stringToArray(value)
    updateSelectedFic({ fic: row, update: { personal_tags: newTags } })
  }
  ...
  const columns: TableColumn<Fic>[] = [
    ...
    },
    {
      ...
      selector: (row) => { return row.personal_tags.toString() },
      sortFunction: (a, b) => {
