...
import {formatFicText} from "../components/Fic/ficFormatters.ts"
import RatingButtons from "../components/Fic/fields/RatingButtons/index.tsx"
import ReadStatusToggle from "../components/Fic/fields/ReadStatusToggle/index.tsx"
import FicLinks from "../components/Fic/fields/FicLinks.tsx"
import AutoComplete from "../components/AutoComplete.tsx"
import FicNotes from "../components/Fic/fields/FicNotes.tsx"

interface props{
    fics: Fic[],
    settings:settingsData,
    allTags: string[],
    updateAllFics: (fics:Fic[])=>void,

}
const Test = ({fics,settings,allTags,updateAllFics}:props) => {
    if(!fics[0])return;
    const fic = fics[0]
    const chapterString = fic.chapters_published + "/" + fic.chapters_total;

    function updateFic({fic,update}:FicUpdate){
        const updatedFic = {...fic, ...update};
        const updatedFics = (fics.map(f =>
            f.ao3id === fic.ao3id ? updatedFic : f
          ));
        updateAllFics( updatedFics);
    }
    
  return (
    <article className="container grid-md fic-modal">
        <header className="modal-header p-relative">
          ...
          <main className="modal-body">
            ...
            <AutoComplete fic={fic} tags={allTags} 
            onUpdateTags={(tags)=>{updateFic({"fic":fic, "update":{"personal_tags": tags}})}  } />
          </main>
          <footer id="fm-footer" className="modal-footer mt-2">
            <Button color="secondary" variant="muted"  label="Close" onClick={()=>{}} />
          </footer>
    </article>
  )
}
